...
from deepin.src.utils import shape_utils
...
class ObjectDetection(model.Model):
    ...
    def postprocess(self, predictions):
        with tf.name_scope('postprocess'):
            predictions_s, detections_s = yolo_v3.decode(predictions['detect_s'], self._anchors[0],
                                                         self._num_classes, self._strides[0])
            predictions_m, detections_m = yolo_v3.decode(predictions['detect_m'], self._anchors[1],
                                                         self._num_classes, self._strides[1])
            predictions_l, detections_l = yolo_v3.decode(predictions['detect_l'], self._anchors[2],
    ...
    def build_model(self, input_queue):
        ...
        if self._visualize_images:
            ...
            bboxes2 = postprocessed_detections  # bbox:xywh
            bboxes2_with_score_and_cls = yolo_v3.nms(bboxes2, confidence_threshold=.4, iou_threshold=.45,
                                                     num_class=self._num_classes)  # bbox: yxyx
            bboxes2_with_score_and_cls = tf.map_fn(
                lambda x: shape_utils.pad_or_clip_nd(tf.boolean_mask(x, x[..., 4] > 0), [50, 6]),
