...
#yhy
def optimize_clones_custom(optimizer, 
                           clone_scope_list,
                           clone_device_list,
                           clone_loss_list,
                           clone_varlist_list,
    ...
    grads_and_vars = []
    losses = []
    num_clones = len(clone_scope_list)
    for i in range(num_clones):
        clone_scope = clone_scope_list[i]
        clone_device = clone_device_list[i]
        clone_loss = clone_loss_list[i]
        clone_varlist = clone_varlist_list[i]
        clone_grads_and_vars = None
        with tf.name_scope(clone_scope):
            if clone_loss is not None:
                if num_clones > 1:
                    clone_loss = tf.div(clone_loss, 1.0 * num_clones, name='scaled_clone_loss')
                with tf.device(clone_device):
                    clone_grads_and_vars = optimizer.compute_gradients(clone_loss, clone_varlist, **kwargs)
                losses.append(clone_loss)
                grads_and_vars.append(clone_grads_and_vars)
    # Compute the total_loss summing all the clones losses.
    total_loss = tf.add_n(losses, name='total_loss')
    # Sum the gradients across clones.
    total_grads_and_vars = _sum_clones_gradients(grads_and_vars)
    return total_loss, total_grads_and_vars
