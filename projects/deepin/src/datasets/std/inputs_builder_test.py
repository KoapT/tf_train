...
from deepin.src.datasets.std import inputs_builder
import time
import os
import numpy as np
import cv2
...
from deepin.src.datasets.std import det_aug_utils
from deepin.src.utils import preprocessor
...
def _preprocess_fn(sample, fixed_height, fixed_width,fast_mode=False,build_label_from_name=(not FLAGS.include_detect_label)):
    ...
    t0 = time.time()
    # det_aug_utils.PIECEWISEAFFINE['probability'] = .0
    # aug = det_aug_utils.Augment(image, num_objs, bboxes, fixed_height, fixed_width, max_num_objects=100)
