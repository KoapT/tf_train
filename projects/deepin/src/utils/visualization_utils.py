...
def draw_bounding_box_on_image(image,
  ...
  try:
    font = ImageFont.truetype(font_path, font_size)#ImageFont.truetype('arial.ttf', 24)
  except IOError:
...
def visualizeSingleLabelClassifications(image_tensor, label_tensor, category_map,
                                        font_size=12):
    """Draws label on batch of image tensors.
    Args:
        image_tensor: A 4D uint8 image tensor of shape [N, H, W, C], C=3. 
            If C > 3, additional channels will be ignored.
        label_tensor: [N, 1] or [N] int tensor of class ids.
        category_index: a dict that maps integer ids to category dicts. e.g.
            {0: {'name': 'dog'}, 1: {'name': 'cat'}, ...} or 
            {1: {'name': 'dog'}, 2: {'name': 'cat'}, ...}
        font_size: font size of label drawn on image
    Returns:
        4D image tensor of type uint8, with label drawn on top.
    """
    shape = image_tensor.get_shape().as_list()
    images = tf.image.grayscale_to_rgb(image_tensor) if shape[3] == 1 else image_tensor
    boxes = tf.zeros([shape[0], 1, 4])
    scores = tf.add(tf.zeros([shape[0], 1]), tf.constant(1.0))
    shift = 1 if min(category_map.keys()) == 0 else 0
    classes = tf.expand_dims(label_tensor, axis=1) if label_tensor.shape.ndims == 1 else label_tensor
    classes = tf.add(classes, tf.constant(shift))
    category_index = {}
    for key in list(category_map.keys()):
        category_index[key+shift] = category_map[key]
    vis_image_tensor = draw_bounding_boxes_on_image_tensors(
            images, boxes, classes, scores, category_index,
            line_thickness=1, 
            font_size=font_size, 
            skip_scores=True)
    return vis_image_tensor

def visualizeMultiLabelClassifications(image_tensor, label_tensor, category_map,
                                       font_size=12):
    """Draws label on batch of image tensors.
    Args:
        image_tensor: A 4D uint8 image tensor of shape [N, H, W, C], C=3. 
            If C > 3, additional channels will be ignored.
        label_tensor: [N, num_classes(exclude id 0)] int tensor of class id lists,
            e.g. [[1,4,0,0],[2,0,0,0],...].
        category_index: a dict that maps integer ids(start with 1) to category dicts. e.g.
            {1: {'name': 'dog'}, 2: {'name': 'cat'}, ...}
        font_size: font size of label drawn on image
    Returns:
        4D image tensor of type uint8, with label drawn on top.
    ...
    images = tf.image.grayscale_to_rgb(image_tensor) if shape[3] == 1 else image_tensor
    labels = label_tensor
    def _func_np(images, labels, 
                 category_index=category_map, 
                 font_size=font_size,
                 color='red'):
        for i in range(np.shape(labels)[0]):
            ...
            for j in range(len(label)):
                if label[j] in category_index.keys():
                    display_str = display_str + category_index[label[j]]['name'] + ','
            ...
            draw_bounding_box_on_image_array(
                    ...
                    font_size=font_size)
        return images
    vis_image_tensor = tf.py_func(_func_np, [images, labels], tf.uint8)
    return vis_image_tensor

def visualizeClassifications(image_tensor, label_tensor, category_map,
                             font_size=12, is_single_label=True):
    if is_single_label:
        vis_image_tensor = visualizeSingleLabelClassifications(
                image_tensor, label_tensor, category_map, font_size)
    else:
        vis_image_tensor = visualizeMultiLabelClassifications(
                image_tensor, label_tensor, category_map, font_size)
    return vis_image_tensor

def visualizeSegmentations(label_mask, label_palette):
    label_mask = tf.cast(label_mask, dtype=tf.int32)
    if label_mask.shape.ndims == 4:
        label_mask = tf.squeeze(label_mask, axis=-1)
    visualize_mask = tf.cast(
            tf.gather(label_palette, label_mask), 
            tf.uint8)
